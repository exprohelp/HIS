...
$(document).ready(function () {
    ...
    $('#tblDonorProcessInfo tbody').on('click', 'i', function () {
        var elem = $(this).closest('td').index() - 10;
        getProcess($(this));
        if (elem == 1) {
            CollectionInfo();
...
function EnableDiableApprove() {
    ...
    if (arrPath[arrPath.length - 1] == 'WholeBloodProcess') {
        if (_IsScreenedApproved == 'Y') {
            $('div[class*=action-btn]').hide();
            $('button[class*=approve]').hide();
            $('#lblApproveStatus').text('Donation Approved').addClass('text-warning').show();
        }
        else {
...
function InsertScreeningInfo() {
    if (confirm('Are you sure?')) {
        if (ValidateScreen()) {
            ...
            objBO.login_id = Active.userId;
            objBO.Logic = 'WholeBloodScreening';
            $.ajax({
                method: "POST",
                url: url,
                data: JSON.stringify(objBO),
