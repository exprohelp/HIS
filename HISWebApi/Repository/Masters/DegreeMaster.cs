...
namespace HISWebApi.Repository.Masters
{
    ...
    {
        public string mInsertUpdateDegree(DegreeBO obj)
        {
            ...
            using (SqlConnection con = new SqlConnection(GlobalConfig.ConStr_Hospital))
            {
                using (SqlCommand cmd = new SqlCommand("pm_tinymaster", con))
                {
                    ...
                    cmd.Parameters.Add("@prm_1", SqlDbType.VarChar, 100).Value = obj.DegName;
                    cmd.Parameters.Add("@prm_2", SqlDbType.VarChar, 500).Value = obj.DegDesc;
