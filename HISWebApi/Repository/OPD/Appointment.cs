...
namespace HISWebApi.Repository.OPD
{
    ...
    {
        public dataSet Opd_DisplayTVQueries(ipAppointment objBO)
        {
            dataSet dsObj = new dataSet();
            using (SqlConnection con = new SqlConnection(GlobalConfig.ConStr_Hospital))
            {
                using (SqlCommand cmd = new SqlCommand("pOpd_DisplayTVQueries", con))
                {
        ...
        public dataSet Opd_ExaminationRoomQueries(AppointmentQueries objBO)
        {
            dataSet dsObj = new dataSet();
            using (SqlConnection con = new SqlConnection(GlobalConfig.ConStr_Hospital))
            {
                using (SqlCommand cmd = new SqlCommand("pOpd_ExaminationRoomQueries", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandTimeout = 2500;
                    cmd.Parameters.Add("@Hosp_Id", SqlDbType.VarChar, 10).Value = objBO.Hosp_Id;
        ...
        public string Opd_InOutMarking(OpdInOutMarking objBO)
        {
            string processInfo = string.Empty;
            using (SqlConnection con = new SqlConnection(GlobalConfig.ConStr_Hospital))
            {
                con.Open();
                using (SqlCommand cmd = new SqlCommand("pOpd_InOutMarking", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandTimeout = 2500;
                    cmd.Parameters.Add("@DoctorId", SqlDbType.VarChar, 20).Value = objBO.DoctorId;
                    cmd.Parameters.Add("@BookingNo", SqlDbType.VarChar, 30).Value = objBO.BookingNo;
                    cmd.Parameters.Add("@inputDate", SqlDbType.DateTime, 10).Value = objBO.inputDate;
